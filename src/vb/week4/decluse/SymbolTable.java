...
public class SymbolTable<Entry extends IdEntry> {
	...
	private ArrayList<Map<String, Entry>> table;
     ...
     */
    public SymbolTable() { 
        table = new ArrayList<Map<String, Entry>>();
    }
    ...
    public void openScope()  {
        table.add(new HashMap<String, Entry>());
    }
    ...
    public void enter(String id, Entry entry)
    	...
    	// Scope not valid
    	if (table.size() == 0) {
    		throw new SymbolTableException("Open up a scope first, dummy!");
    	}
    	// Already declared
    	if (table.get(table.size() - 1).get(id) != null) {
    		throw new SymbolTableException(id + " is already there somewhere...");
    	}
    	
    	// Put it there
    	Map<String, Entry> map = table.get(table.size() - 1);
    	map.put(id, entry);
    }
    ...
    public Entry retrieve(String id) {
    	Entry entry = null;
    	for (int i = table.size() - 1; i >= 0 && entry == null; i--) {
    		entry = table.get(i).get(id);
    	}
